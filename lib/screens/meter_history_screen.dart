...
class _MeterHistoryScreenState extends State<MeterHistoryScreen> {
  ...
  bool _huboDatosReales(Map<String, dynamic> dataRoot) {
    ...
    final totalConsumption =
        (dataRoot['totalConsumption'] as num?)?.toDouble() ?? 0;
  ...
  Future<void> _fetchAIPrediction() async {
    ...
    await Future.delayed(
        const Duration(milliseconds: 600)); // sensación de "cálculo"
    ...
    setState(() {
      _aiPrediction = _AIPrediction(
        ...
        message:
            'Estimación basada en tendencia de consumo de los últimos $diasTranscurridos días.',
      );
      _aiLoading = false;
    });
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
              'Variaciones puntuales registradas directamente por el sensor.',
  ...
  Widget _buildBarChart() {
    ...
    if (m == null || m.chartData.isEmpty) {
      return _emptyChart(
          m?.message ?? 'Sin registros de consumo en este periodo.');
    }
    ...
    double ultimoValorValido =
        m.averagePercentage > 0 ? m.averagePercentage : 100.0;

    for (int i = 0; i < 4; i++) {
      if (semanasValores[i].isNotEmpty) {
        semanasPromedios[i] = semanasValores[i].reduce((a, b) => a + b) /
            semanasValores[i].length;
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Nivel de gas promedio semanal',
              style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
          const Text('Monitoreo macro — % promedio del tanque',
          ...
          SizedBox(
            ...
            child: Row(
              ...
              children: List.generate(4, (index) {
                ...
                final barH =
                    (barMaxH * (porcentajeSemana / 100.0)).clamp(6.0, barMaxH);

                bool hayDecremento = index > 0 &&
                    semanasPromedios[index] < semanasPromedios[index - 1];
  ...
  Widget _buildLiterCards() {
    ...
    return Row(
      children: [
        Expanded(
            child: _literCard(
                valor: '${consumidoLt.toStringAsFixed(1)} lt',
                label: 'Consumido',
                color: MeterHistoryScreen.primaryBlue)),
        const SizedBox(width: 12),
        Expanded(
            child: _literCard(
                valor: '${restanteLt.toStringAsFixed(1)} lt',
                label: 'Restante prom.',
                color: MeterHistoryScreen.primaryBlue)),
      ],
    );
    ...
    if (m == null || m.logs.isEmpty) {
      return Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(color: const Color(0xFFE0E0E0))),
        child: const Center(
            child: Text('Sin logs de lecturas disponibles.',
  ...
  Widget _buildAIPredictionSection() {
    ...
    if (pred == null || !pred.hasPrediction) {
      return const Text(
          'No se pudo generar una predicción con los datos actuales.',
        ...
        children: [
          Row(
            children: [
              ...
              const Text('PREDICCIÓN GENERADA',
                  style: TextStyle(
                      color: Colors.white,
                      fontSize: 12,
                      fontWeight: FontWeight.bold,
          ...
          const SizedBox(height: 16),
          Text('PRÓXIMA RECARGA ESTIMADA: En ${pred.daysRemaining} días',
              style: const TextStyle(
                  color: Colors.white,
                  fontSize: 15,
                  fontWeight: FontWeight.bold)),
          const SizedBox(height: 6),
          Text('CONSUMO CALCULADO: ${pred.estimatedConsumptionRate ?? "—"}',
              style: const TextStyle(color: Colors.white70, fontSize: 13)),
          Text(
              'NIVEL DE CONFIANZA: ${((pred.confidenceScore ?? 0) * 100).toStringAsFixed(0)}%',
              style: TextStyle(
          ...
          Text(
              pred.message ??
                  'Análisis predictivo completado con éxito basado en tus flujos de dispersión.',
              style: TextStyle(
                  color: Colors.blue[100], fontSize: 11, height: 1.3)),
        ],
      ),
    );
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final log in logs) {
      ...
      final dotPaint = Paint()
        ..color = isOutlier
            ? Colors.red
            : (log.percentage < 25 ? Colors.amber[700]! : lightBlue)
